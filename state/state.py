...
class State:
    ...
    yaw = pitch = roll = 0 # Positive Pitch = Right. Negative Roll = Forward

    def __init__(self, imu=imu, T=0.05):
        # Update period
        self.T = T
    ...
    def update(self):
        ...
        x,y,z,w = self.q
        self.yaw   =  atan2(2*x*y + 2*w*z, w*w + x*x - y*y - z*z)/pi*180.0
        self.pitch = -asin(2*w*y - 2*x*z)/pi*180.0
        self.roll  = -atan2(2*y*z + 2*w*x, -w*w + x*x + y*y - z*z)/pi*180.0
